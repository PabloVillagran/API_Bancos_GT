...
this_service.login = function(req, res){
    ...
    axios.post('http://localhost:9000/login',{"usuario": req.body.usuario, "password": req.body.password})
    .then(serviceResponse => {
        let token = mysql.escape(serviceResponse.data.token);
        //Insertar transacción en tabla
        this_service.query("INSERT INTO `sesion_usuario`(`Nombre_usuario`, `Token`, `fecha_hora_expira`, `correo_electrónico`, `API_Banco_idAPI`) VALUES ("+usuario+","+token+",date_add(now(), interval 30 MINUTE),"+correo+",'1')", res);
        res.json({success:true, "token":serviceResponse.data.token});
    })
    .catch(error=>{
        console.log(error);
